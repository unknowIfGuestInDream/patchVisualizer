pipeline {
    ...
    stages {
        ...
        stage('Build patchvisualizer-windows') {
            steps {
                script {
                    packageApp('win')
                }
            ...
            post {
                success {
                    archiveArtifacts 'patchvisualizer*win*.zip'
                }
                failure {
                    echo 'Build patchvisualizer-windows failed'
                }
                aborted {
                    echo 'Build aborted'
                }
        ...
        stage('Build patchvisualizer-mac') {
            steps {
                script {
                    packageApp('mac')
                }
            ...
            post {
                success {
                    archiveArtifacts 'patchvisualizer*mac*.zip'
                }
                failure {
                    echo 'Build patchvisualizer-mac failed'
                }
                aborted {
                    echo 'Build aborted'
                }
        ...
        stage('Build patchvisualizer-linux') {
            steps {
                script {
                    packageApp('linux')
                }
            ...
            post {
                success {
                    archiveArtifacts 'patchvisualizer*linux*.zip'
                }
                failure {
                    echo 'Build patchvisualizer-linux failed'
                }
                aborted {
                    echo 'Build aborted'
                }
...
def packageApp(os) {
    ...
    sh "cd staging && zip -qr ../patchvisualizer-${os}_${version}_b${BUILD_NUMBER}_\$(date +%Y%m%d).zip . && cd .."
    sh "rm -rf staging"
}
